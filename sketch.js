...
function preload(){
playerImg=loadAnimation("image/player.png","image/player-frame2.png","image/playerImg-3.png");
playerImg2=loadAnimation("image/dan.png","image/lplayer-frame2.png","image/lplayerImg-3.png");
heatblast=loadAnimation("image/heat.png","image/heatblast.png","image/heatImg.png");
lheatblast=loadAnimation("image/lheat.png","image/lheatblast.png","image/lheatImg.png");
...
heatbeatImg=loadImage("image/heatbeat.png");
heatbeatImg2=loadImage("image/lheatbeat.png");
...
//alert("If the game is laging so please restart the game and please give ratings because this is my new game so please tell me how it is")
...
if(gameState != "start" && gameState != bonus && gameState != level2 && gameState != level6 && gameState != level11){
  player.setCollider("rectangle", 0, 0+7, 50, 100, 0);
}


if(gameState != "start" && gameState != bonus && gameState === level2 || gameState === level6 || gameState === level11){
  player.setCollider("rectangle", 0, 0, 19, 35, 0);
  //player.debug=true;
}
...
player=createSprite(windowWidth/2-100,windowHeight/2);
player.addAnimation("running",heatblast);
player.scale=3;
...
e30 = new Enemy(windowWidth/2+5700,windowHeight/2)
e31 = new Enemy(windowWidth/2+5900,windowHeight/2)
...
rightbutton=createSprite(windowWidth/2-250,windowHeight/2+110);
rightbutton.addImage(rightImg);
rightbutton.scale=0.3;

leftbutton=createSprite(windowWidth/2-350,windowHeight/2+110);
leftbutton.addImage(leftImg);
leftbutton.scale=0.3;

punchButton=createSprite(windowWidth/2+350,windowHeight/2+110);
punchButton.addImage(punchImg);
punchButton.scale=0.240;

jumpButton=createSprite(windowWidth/2+250,windowHeight/2+110)
jumpButton.addImage(jumpImg);
jumpButton.scale=0.3;
  ...
  if (r.isHeld){
    ...
    if(frameCount % 16 === 0 && move === 5){
     ...
     if (l.isHeld){
      angle=4;
      if(frameCount % 16 === 0 && move === 5){
      player.addAnimation("running",lheatblast);
     }
...
health.width=health.width-5;
    ...
    if(player.isTouching(wall1) && wall1.x === windowWidth/2+4400){
      ...
      e3 = new Enemy(windowWidth/2+5200,windowHeight/2);
      e4 = new Enemy2(windowWidth/2+5400,windowHeight/2);
      e5 = new Enemy(windowWidth/2+5600,windowHeight/2);
...
function punch(){
  ...
  once2=4;
    ...
    if(angle === 5){
      ...
      if(gameState != level2 || gameState != level6 || gameState != level11 ){
  player.addAnimation("running",beatImg);
      }

      if(gameState === level2 || gameState === level6 || gameState === level11){
        player.addAnimation("running",heatbeatImg);
            }
   ...
   if(angle === 4){
    ...
    if(gameState != level2 || gameState != level6 || gameState != level11){
      player.addAnimation("running",beatImg2);
          }
    
          if(gameState === level2 || gameState === level6 || gameState === level11){
            player.addAnimation("running",heatbeatImg2);
                }
